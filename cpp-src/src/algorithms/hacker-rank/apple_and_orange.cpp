...
{
    auto hSize = t - s;
    auto itA = std::partition(begin(apples), end(apples),
                              [&](int d) -> bool {
                            int x=s-a-d;
                            return (x <=0) && (std::abs(x)<=hSize); });
    cout << std::distance(begin(apples), itA) << "\n";
    auto itO = std::partition(begin(oranges), end(oranges),
                              [&](int d) -> bool {
                            int x=b+d-t;
                            return (x <=0) && (std::abs(x)<=hSize); });
    cout << std::distance(begin(oranges), itO) << "\n";
}
